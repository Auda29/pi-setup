...
function Test-PiSetupManagedFile {
    param([Parameter(Mandatory = $true)][string]$Path)
    ...
    try {
        $raw = Get-Content -LiteralPath $Path -Raw -Encoding UTF8
        return $raw -match 'Generated by pi-setup'
    }
    catch {
        Write-Warning "Could not inspect generated file '$Path': $($_.Exception.Message)"
        return $false
    }
}
...
function Set-GeneratedFileContent {
    ...
    if ((Test-Path -LiteralPath $Path) -and -not (Test-PiSetupManagedFile -Path $Path)) {
...
exit `$LASTEXITCODE
"@
    Set-GeneratedFileContent -Path $StartScriptPath -Content $startScript -Encoding UTF8
